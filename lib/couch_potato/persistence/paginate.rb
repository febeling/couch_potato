      ...
      module ClassMethods
        def paginate(page = 1, per_page = 15, options = {})
          ...
          WillPaginate::Collection.create(page, per_page) do |pager|
            ...
            ids, total = find_page_ids_ordered_by(page, per_page, options[:order], options[:descending], clazz ? clazz.name : nil)
        ...
        def paginate_map_function(keys, type, type_attribute='ruby_class')
          key_ary = []
          case keys
          when Array
            key_ary += keys
          ...
          "function(doc) {
              #{type_predicate((type.nil? ? nil : type), type_attribute)} emit([#{key_ary.map{|k|"doc.#{k}"}.join(', ')}], null);
           }"
        end
